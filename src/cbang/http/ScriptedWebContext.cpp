...
ScriptedWebContext::ScriptedWebContext(ScriptedWebHandler &handler,
                                       Connection &con) :
  WebContext(handler, con), handler(handler) {}


ScriptedWebContext::~ScriptedWebContext() {} // Hide destructor implementation


Script::Environment &ScriptedWebContext::getEnvironment() {
  if (env.isNull()) env = new Script::Environment("HTTP Context", &handler);
  return *env;
}


bool ScriptedWebContext::eval(const Script::Context &ctx) {
  // NOTE This dispatch is used instead of mapping the eval functions
  //   directly in to the ScriptedWebContext to avoid the cost of mapping them
...
void ScriptedWebContext::evalInclude(const Script::Context &ctx) {
...
void ScriptedWebContext::evalGet(const Script::Context &ctx) {
...
void ScriptedWebContext::evalRemoteAddr(const Script::Context &ctx) {
  ctx.stream << con.getClientIP();
}


void ScriptedWebContext::evalRequestURI(const Script::Context &ctx) {
  ctx.stream << con.getRequest().getURI();
}


void ScriptedWebContext::evalRequestMethod(const Script::Context &ctx) {
  ctx.stream << con.getRequest().getMethod();
}


void ScriptedWebContext::evalRequestQuery(const Script::Context &ctx) {
  ctx.stream << con.getRequest().getURI().getQuery();
}


void ScriptedWebContext::evalRequestPath(const Script::Context &ctx) {
