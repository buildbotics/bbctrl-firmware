...
void XMLSkipHandler::startElement(const string &name,
                                  const XMLAttributes &attrs) {
  if (++depth != 1) parent.startElement(name, attrs);
}


void XMLSkipHandler::endElement(const string &name) {
  if (--depth != 0) parent.endElement(name);
}
